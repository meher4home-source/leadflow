...
import { runQualifyingStep } from '../../lib/ai.js';
import { fireUniversalWebhook } from '../../lib/webhook.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: recentOutcomes } = await svc
      .from('leads')
    ...
    if (step.done) {
      const newClass = step.classification || 'mid_range';
      await svc
        .from('leads')
        .update({
      ...
      if (profile.webhook_url) {
        fireUniversalWebhook(profile.webhook_url, 'lead.classified', { lead_id: lead.id, name: lead.name, classification: newClass, estimated_value: step.estimated_value });
      }
    }
