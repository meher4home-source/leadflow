 ...
 */
export async function runQualifyingStep({ industryKey, businessName, bookingLink, conversation, recentOutcomes = [] }) {
  ...
  if (recentOutcomes.length > 0) {
    const converted = recentOutcomes.filter(r => r.outcome === 'converted').length;
    outcomesContext = `\n(Recent performance context: Out of the last ${recentOutcomes.length} qualified leads, ${converted} converted. Use this to subtly guide your sense of what a good lead looks like for this specific business, but don't mention this to the lead.)\n`;
  }
