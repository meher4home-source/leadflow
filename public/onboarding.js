...
async function resumeAfterAuth() {
  ...
  const storedRef = localStorage.getItem('lf_ref_code');
  if (storedRef) {
    const profile = await LF.getProfile(user.id);
    if (profile && !profile.referred_by) {
      await LF.client.from('profiles').update({ referred_by: storedRef }).eq('id', user.id);
    }
    localStorage.removeItem('lf_ref_code');
  }
...
(async function init() {
  ...
  if (refCode) {
    localStorage.setItem('lf_ref_code', refCode);
  }
